...
def create_subdomain(subdomain):
    """Create a subdomain using Cloudflare API, update if it already exists"""
    ...
    print(f"Checking if {full_domain} already exists...")
    list_url = f"https://api.cloudflare.com/client/v4/zones/{CF_ZONE_ID}/dns_records?type=A&name={full_domain}"
    ...
    try:
        ...
        record_id = None
        if list_response.status_code == 200 and list_json.get("success"):
            records = list_json.get("result", [])
            if records:
                record_id = records[0]["id"]
                print(f"Found existing record for {full_domain} with ID: {record_id}")
...
def setup_ssl(domain, use_dns_cloudflare=True):
    ...
    if result.returncode == 0:
        ...
        if use_dns_cloudflare:
            ...
            msg = "4. Try using HTTP challenge as a fallback:"
            cmd = (
                "proxy-manager setup --subdomain yourdomain --local-port xxxx --allowed-ip x.x.x.x --use-http-challenge"
            )
            print(f"{msg} {cmd}")
        else:
...
def main():
    ...
    setup_parser.add_argument(
        "--use-http-challenge",
        action="store_true",
        help="Use HTTP-01 challenge instead of DNS-01 for certificate issuance",
    )
    ...
    subparsers.add_parser("env", help="Check environment variables")
        ...
        if subdomain_success:
            ...
            if config_success:
                ...
                if ssl_success:
                    ...
                    print(
                        f"To create a tunnel, run: proxy-manager tunnel --local-port {args.local_port} "
