...
def create_config(domain, local_port, allowed_ip):
    """Create nginx configuration from a template"""
...
def main():
    ...
    # Setup command
    setup_parser = subparsers.add_parser("setup", help="Set up a new reverse proxy")
    ...
    setup_parser.add_argument("--allowed-ip", required=True, help="IP is allowed to access the proxy")
        ...
        if create_subdomain(args.subdomain):
            if create_config(full_domain, args.local_port, args.allowed_ip):
                if setup_ssl(full_domain):
                    ...
                    print(
                        f"To create a tunnel, run: ./cli.py tunnel --local-port {args.local_port} --remote-port <remote_port>")
